...
if(isset($_POST['submit']))
{
    ...
    if (($con->query($mysql)) == TRUE) 
        {
            echo "<script type='text/javascript'>alert('Appointment Completed');</script>";
            // header(DoctorHome.php);
        } 
        else 
        {
            echo "<script type='text/javascript'>alert('Appointment unsuccessfully');</script>";
        }
}
...
<body class="ip-body">
    ...
    <?php
        ...
        {
            // $mysql = "SELECT * FROM appointments WHERE status ='active' AND patientID = '$_POST[search]' ";
            // $sql1 = mysqli_query($con, $mysql);
            ...
            $pid = $_POST['search'];
             $sql = "SELECT * FROM appointments WHERE status = 'active' AND patientID = '$pid' ";
             ...
             { 
                 ...
                 <th> $res[symptoms] </th>
                 <th> $r_pn[history] </th>
                 <th><button class ='complete' type='submit' name='submit' id='submit' value='$res[appointmentID]'/>Complete</button></th>
                 </form>
                 </tr>";
             }
        echo "</table>";
